...
int bbutil_load_texture(const char* filename, int* width, int* height, float* tex_x, float* tex_y, unsigned int *tex, png_byte **raw_pix) {
    ...
    printf("rowbytes:%d, image_height:%d", rowbytes, image_height);fflush(stdout);
    ...
    printf("raw_textw:%d, raw_texth:%d", tex_width, tex_height);
    GLint err;
    if(raw_pix == 0){
		glGenTextures(1, tex);
    	...
    	printf("create_image::returning raw pix\n");fflush(stdout);
    	*raw_pix = image_data;
    	err = 0;
    }
    //clean up memory and close stuff
...
UIQuad * create_text(font_t *font, float x, float y, char * msg){
	...
	for(i = 0; i < msg_len; ++i) {
		...
		quad->vertices[8 * i + 0] = x + pen_x + font->offset_x[c];
		quad->vertices[8 * i + 1] = y + font->offset_y[c];
		quad->vertices[8 * i + 2] = quad->vertices[8 * i + 0] + font->width[c];
		quad->vertices[8 * i + 3] = quad->vertices[8 * i + 1];
		quad->vertices[8 * i + 4] = quad->vertices[8 * i + 0];
		quad->vertices[8 * i + 5] = quad->vertices[8 * i + 1] + font->height[c];
	...
	for(i = 0; i < 4 * msg_len; ++i) {
		quad->vertices[2 * i + 0] = 2 * quad->vertices[2 * i + 0] / 1024 - 1.0f;
		quad->vertices[2 * i + 1] = 2 * quad->vertices[2 * i + 1] / 600 - 1.0f;
		//printf("text_vert1:%f,text_vert2:%f\n", quad->vertices[2 * i + 0], quad->vertices[2 * i + 1]);
	}
...
UIQuad *create_image(const char* filename, float x, float y,  int raw_pix){
	...
	if(raw_pix == 1){
		printf("create_image: Raw Pixels requested");fflush(stdout);
	...
	printf("Image Created: w:%f, h:%f, texW:%f, texH:%f", quad->w, quad->h, quad->textureW, quad->textureH);fflush(stdout);
...
//		{{1.0,(float)quad->h/600.0f}  ,{1.0f,1.0f}}, //top right
//		{{-1.0,(float)quad->h/600.0f} ,{0.0f,1.0f}}, //top left
