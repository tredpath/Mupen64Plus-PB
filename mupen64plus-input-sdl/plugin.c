...
//                DebugMessage(M64MSG_INFO,"Axis Val %i\n",axis_val);
//                if (deadzone < 0)
...
//    	SDL_PumpEvents();
//
//        while(SDL_PeepEvents(&event,1,SDL_GETEVENT,SDL_MOUSEMOTIONMASK|SDL_MOUSEBUTTONDOWNMASK |SDL_MOUSEBUTTONUPMASK) == 1)
//        {
//        	ProcessTouchEvent(&event,&controller[Control],button_bits);
//        }
...
EXPORT void CALL Native_KeyUpDown(void *event){
	//printf("Native Event Handler Activated!!");fflush(stdout);
	if(event == 0){
